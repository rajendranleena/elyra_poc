...
args = {
    "project_id": "first-0707092635",
}

dag = DAG(
    dag_id="first-0707092635",
    default_args=args,
    schedule="@once",
...
op_0ba5d858_1dd3_4d64_b21c_a1024da15f3f = KubernetesPodOperator(
    ...
    volume_mounts=[],
    secrets=[
        Secret("env", "AWS_ACCESS_KEY_ID", "elyra-cos-creds-lr", "AWS_ACCESS_KEY_ID"),
        Secret(
            "env",
...
op_bea2611e_291c_4c36_b5a0_7a8cf2d49453 = KubernetesPodOperator(
    ...
    cmds=["sh", "-c"],
    arguments=[
        "mkdir -p ./jupyter-work-dir/ && cd ./jupyter-work-dir/ && echo 'Downloading https://raw.githubusercontent.com/elyra-ai/elyra/v4.1.1/elyra/airflow/bootstrapper.py' && curl --fail -H 'Cache-Control: no-cache' -L https://raw.githubusercontent.com/elyra-ai/elyra/v4.1.1/elyra/airflow/bootstrapper.py --output bootstrapper.py && echo 'Downloading https://raw.githubusercontent.com/elyra-ai/elyra/v4.1.1/etc/generic/requirements-elyra.txt' && curl --fail -H 'Cache-Control: no-cache' -L https://raw.githubusercontent.com/elyra-ai/elyra/v4.1.1/etc/generic/requirements-elyra.txt --output requirements-elyra.txt && python3 -m pip install packaging && python3 -m pip freeze > requirements-current.txt && python3 bootstrapper.py --pipeline-name 'first' --cos-endpoint https://s3.us-east-1.amazonaws.com --cos-bucket nx1poc-pdc-default-ygglold --cos-directory 'first-0707092635' --cos-dependencies-archive '02_transform-bea2611e-291c-4c36-b5a0-7a8cf2d49453.tar.gz' --file 'utils/leena/elyra/02_transform.ipynb' --inputs 'raw_sales.csv' --outputs 'summary_by_region.csv' "
    ],
...
op_06d1bd3e_6545_4955_b41b_8c2433737ecf = KubernetesPodOperator(
    ...
    cmds=["sh", "-c"],
    arguments=[
        "mkdir -p ./jupyter-work-dir/ && cd ./jupyter-work-dir/ && echo 'Downloading https://raw.githubusercontent.com/elyra-ai/elyra/v4.1.1/elyra/airflow/bootstrapper.py' && curl --fail -H 'Cache-Control: no-cache' -L https://raw.githubusercontent.com/elyra-ai/elyra/v4.1.1/elyra/airflow/bootstrapper.py --output bootstrapper.py && echo 'Downloading https://raw.githubusercontent.com/elyra-ai/elyra/v4.1.1/etc/generic/requirements-elyra.txt' && curl --fail -H 'Cache-Control: no-cache' -L https://raw.githubusercontent.com/elyra-ai/elyra/v4.1.1/etc/generic/requirements-elyra.txt --output requirements-elyra.txt && python3 -m pip install packaging && python3 -m pip freeze > requirements-current.txt && python3 bootstrapper.py --pipeline-name 'first' --cos-endpoint https://s3.us-east-1.amazonaws.com --cos-bucket nx1poc-pdc-default-ygglold --cos-directory 'first-0707092635' --cos-dependencies-archive '03_report-06d1bd3e-6545-4955-b41b-8c2433737ecf.tar.gz' --file 'utils/leena/elyra/03_report.ipynb' --inputs 'summary_by_region.csv;raw_sales.csv' --outputs 'report.txt' "
    ],
    task_id="03_report",
    env_vars={
        "ELYRA_RUNTIME_ENV": "airflow",
